...
export default function ButtonCounter({ decreaseQuantity, increaseQuantity, count }) {
    ...
    return (
        <button className='button  button--cta_counter  button--abs_position  button--bg_red' type='button'>
            <div className='button__container  button__container--space_between'>
                <img className='decrease_sign' src={iconDecreaseCount} alt='decrease quantity sign' onClick={decreaseQuantity} />
                <span className='text_white_color  text_preset_4--bold'>{count}</span>
                <img className='increase_sign' src={iconIncreaseCount} alt='increase quantity sign' onClick={increaseQuantity} />
            </div>
        </button>
    )
}